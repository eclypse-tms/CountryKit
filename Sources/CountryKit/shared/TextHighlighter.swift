...
public protocol TextHighlighter: AnyObject {
    /// the color to use when highlighting the search parameters in the CountryPicker UI
...
open class TextHighlighterImpl: TextHighlighter {
    ...
    public func highlightedText(sourceText: String, searchComponents: [String]) -> NSAttributedString {
        ...
        let _highlightColor = highlightColor ?? (UIColor(named: "color_search_text_background", in: CountryKit.assetBundle, compatibleWith: nil) ?? UIColor.systemYellow)
        ...
        for eachTextToSearch in searchComponents {
            ...
            if let rangeCandidate = sourceText.localizedStandardRange(of: eachTextToSearch) {
                ...
                attributedSourceText.addAttribute(NSAttributedString.Key.backgroundColor, value: _highlightColor, range: rangeOfTextToHighlight)
